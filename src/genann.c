...
genann *genann_init(RandomState *rng, int inputs, int hidden_layers, int hidden, int outputs)
{
    if (hidden_layers < 0)
        return 0;
    if (inputs < 1)
...
genann *genann_read(FILE *in)
{
    ...
    genann *ann = genann_init(NULL, inputs, hidden_layers, hidden, outputs);
...
void genann_randomize(RandomState *rng, genann *ann)
{
    int i;
    for (i = 0; i < ann->total_weights; ++i)
    {
